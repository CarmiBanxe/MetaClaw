...
- **[2026/03/13]** **v0.3** — 메타러닝 스케줄러: RL 가중치 업데이트를 수면 시간, 유휴 시간, Google Calendar 회의 중에만 실행. MAML 방식의 support/query 집합 분리를 추가하여 오래된 보상 신호가 모델 업데이트를 오염시키는 것을 방지.
...
| 모드 | 기본값 | 기능 |
|------|--------|------|
| `auto` | ✅ | RL + 스마트 스케줄러. 스킬은 항상 활성화. RL 가중치 업데이트는 수면/유휴/회의 창에서만 실행. |
...
RL 모드에서 가중치 핫스왑 단계는 에이전트를 수 분간 일시 정지시킵니다. 스케줄러(`auto` 모드에서 기본 활성화)는 RL 업데이트를 사용자 비활성 창으로 연기하여 활성 사용 중에 에이전트가 중단되지 않도록 합니다.
