 ...
 *
 * Утверждения, помеченные как ДЕФЕКТ, обязаны стать КРАСНЫМИ, когда обязательность модели,
 * проверки и надзора станет СЕРВЕРНОЙ ПОЛИТИКОЙ, а не полями запроса (`S3-07`): тогда
 * `requiresModel/Review/Oversight: false` перестанет что-либо значить, и `enforced` при нуле
 * квитанций станет `false`. Красный цвет здесь будет доказательством починки.
 */
