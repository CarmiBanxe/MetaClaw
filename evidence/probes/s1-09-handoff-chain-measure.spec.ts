 ...
 *
 * ── ПОПРАВКА К ЭТОМУ ЖЕ ЗАГОЛОВКУ, 2026-09-11 ───────────────────────────────────────────────
 *
 * Здесь стояло: «Две первые пробы КРАСНЫ и оставлены красными НАМЕРЕННО: они закрепляют, ГДЕ
 * именно цепь обрывается сегодня». **Это было слишком великодушно к себе.** Красный цвет шёл от
 * `NO_MLRO_JOURNAL` — ПРЕДШЕСТВУЮЩЕГО отказа, — и такой красный закрепляет не место обрыва, а
 * то, что проба до предмета НЕ ДОХОДИТ. Я записала ловушку в заголовок и осталась в ней.
 *
 * Найдено сплошным прогоном всех тридцати одной пробы: она оказалась единственной красной, и
 * причина была не та, которую называл заголовок.
 *
 * Журнал второй линии подан сборкой — последнее предусловие перед местом, где контракта нет.
 * Проба дошла до названного предмета, и находка ОКАЗАЛАСЬ СИЛЬНЕЕ прежней:
 *
 *   S1-09 запрещённое поле → 200 {"decided":true,"case_id":"case_1","status":"APPROVED", …}
 *   S1-09 состояние после нарушения границы → APPROVED
 *
 * То есть поле из закрытого перечня `FORBIDDEN_RESPONSE_FIELDS` не только проходит дверь —
 * решение ПРИНИМАЕТСЯ и состояние дела МЕНЯЕТСЯ. Три пробы зелены: две закрепляют дефект,
 * третья — контроль.
 *
 * ЧТО СДЕЛАЕТ ЭТУ ПРОБУ КРАСНОЙ: подключение контракта handoff в конец цепи и ДО изменения
 * состояния (`S1-09`, `specs/mlro-handoff-into-executable-path.md` R1). Тогда `200` станет
 * отказом `FORBIDDEN_FIELD_PRESENT`, а состояние останется `PENDING`. Красный здесь будет
 * доказательством починки, и переписывать пробу полагается ВМЕСТЕ с ней.
 */
...
async function дверь() {
  ...
  // РЕДАКЦИЯ 2, 2026-09-11. Прежняя сборка не подавала `mlroJournal`, и дверь отвергала на
  // `NO_MLRO_JOURNAL` — ПРЕДШЕСТВУЮЩЕМ отказе, до названного предмета. Проба была красной, и
  // заголовок звал этот красный «намеренным», закрепляющим место обрыва. Это было слишком
  // великодушно к себе: красный ОТ ПРЕДШЕСТВУЮЩЕГО отказа закрепляет не место обрыва, а то,
  // что проба туда НЕ ДОХОДИТ.
  //
  // Собственное правило (`specs/mlro-handoff-into-executable-path.md`, A2) требует снимать
  // предшествующие отказы подсадкой их условий, пока не заговорит названный. Здесь подан
  // журнал второй линии — последнее предусловие перед местом, где контракта нет.
  const built = buildDirectorServer({} as never, {
    reviewQueue: queue, holders: HOLDERS,
    subjects: { resolve: () => "sim://sandbox/MLRO" },
    mlroJournal: { append: () => undefined },
